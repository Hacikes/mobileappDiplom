...
class WithdrawFromAccountScreen extends StatefulWidget {
  const WithdrawFromAccountScreen({
    Key? key,
  ...
  @override
  State<WithdrawFromAccountScreen> createState() => _WithdrawFromAccountScreentate();
}

class _WithdrawFromAccountScreentate extends State<WithdrawFromAccountScreen> {
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      final withdrawFromAccountService = BuySellInstrumentService();
                      final statusCode = await withdrawFromAccountService.withdrawMoneyFromAccount(
                        CountRUB.toInt(),
                      );
                      switch (statusCode) {
                        case 200:
                        ...
                        // Обработка других статусов ответа
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Упс... Что-то пошло не так ...\nВозможно вы пытаетесь вывести больше,\nчем у вас есть'),
                              duration: Duration(seconds: 3),
                            ),
                          );
                          break;
