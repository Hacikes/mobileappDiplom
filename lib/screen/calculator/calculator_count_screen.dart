...
import 'package:mobile_app_diplom/screen/calculator/buttons_change_calculator_screen/botton_calculator_count.dart';
...
class CalculatorInputVolumeScreen extends StatefulWidget {
  CalculatorInputVolumeScreen({
    Key? key,
  ...
  @override
  State<CalculatorInputVolumeScreen> createState() =>
      _CalculatorInputVolumeScreenState();
}

class _CalculatorInputVolumeScreenState
    extends State<CalculatorInputVolumeScreen> {
  ...
  Future<void> setupTotalVolumeByAccount() async {
    TotalVolumeCurrencyByAccountInRUB instance =
    TotalVolumeCurrencyByAccountInRUB();
    await instance.getTotalVolumeCurrencyByAccountInRUBParameterAccountId(
        accountIdIndex);
    ...
    setState(() {
      ...
      // print("totalVolumeForAccount: $totalVolumeForAccount");
    });
  }
  ...
  Future<void> setupTotalVolumeByUser() async {
    ...
    String totalSum = instance.totalSum;
    String numericValue = totalSum.replaceAll(RegExp(r'[^\d.]'), '');
  ...
  //   TotalQuantity = getCount();
  //   getCount(); // Добавить эту строку
  //   return ((inputPrice * TotalQuantity) / totalVolumeForAccount) * 100;
  // }
  ...
  int getCount() {
    if (inputPercent == 0 || totalVolumeForAccount == 0) {
      inputCount = 0;
    } else {
      inputCount = ((totalVolumeForAccount / inputPrice) * (inputPercent / 100)).round();
      // print("totalVolumeForAccount: $totalVolumeForAccount");
      // print("inputPrice: $inputPrice");
          ...
          Container(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: <Widget>[
                  BottomBuyCalculatorCount(toggleView: widget.toggleView,),
                  ...
                  TextFormField(
                    onChanged: (value) {
                      setState(() {
                        ...
                        // print('inputPrice: ${inputPrice}');
                        getCount();
                        // print('getCount_Стоимость: ${getCount()}');
                      });
                    },
                    inputFormatters: [
                  ...
                  TextFormField(
                    onChanged: (value) {
                      setState(() {
                        inputPercent = double.tryParse(value) ?? 0;
                        // print('inputPercent: ${inputPercent}');
                        getCount();
                        // print('getCount_Ведённый процент: ${getCount()}');
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: 'Процент от счёта',
                      filled: true,
                      fillColor: ColorsClass.getFrontForHintOnField(),
                    ),
                    validator: (val) {
                      if (val!.isEmpty) {
                        return 'Введите процент от счёта';
